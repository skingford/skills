---
name: git-clean-main
description: "Keep AI tooling files (.claude, .codex, openspec, skills-lock.json) on dev branch but exclude them from main/master. Use when managing branches, creating PRs to main, merging to main, or setting up a repo's branch strategy for AI-assisted development. Triggers on git merge/PR operations targeting main or master."
---
...
.cursorignore     ← Cursor editor config
    ...
    steps:
      ...
      - name: Clean merge to main
        run: |
          ...
          for item in .claude .codex openspec skills-lock.json .cursorignore .cursor; do
            git rm -rf --cached "$item" 2>/dev/null || true
            rm -rf "$item" 2>/dev/null || true
          done
...
3. **After merge**, verify AI files are not present on `main`:
   ```bash
   git checkout main
   ls -la .claude .codex openspec skills-lock.json 2>&1
...
# Remove AI files
git rm -rf .claude .codex openspec skills-lock.json .cursorignore .cursor 2>/dev/null
